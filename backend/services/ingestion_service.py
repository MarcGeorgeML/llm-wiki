...
from schema import SCHEMA, INGESTION_PROMPT, REUSE_SCHEMA, EXAMPLES, SELECT_PAGES_SCHEMA
...
class IngestionService(PDFService):
    ...
    def _get_page_map(self) -> dict[str, Path]:
        return {p.stem: p for p in self.WIKI_DIR.rglob("*.md") if p.name != "index.md"}


    def _assign_pages(self, chunk: str, index: str, max_pages: int = 5) -> list[str]:
        if not index:
            return []
        prompt = f"CONTENT:\n{chunk}\n\nEXISTING PAGES:\n{index}\n\nSelect up to {max_pages} page names to update. Return ONLY a JSON array, or []."
        response = "".join(self.stream_fn(prompt, system=SELECT_PAGES_SCHEMA.format(max_pages=max_pages)))
        try:
            selected = json.loads(response)
        except Exception:
            return []
        valid = {line.split("[[")[1].split("]]")[0] for line in index.splitlines() if "[[" in line}
        return [p for p in selected if p in valid][:max_pages]


    def _build_prompt(self, chunk: str, chunk_index: int, total_chunks: int, index: str, selected: list[str], filename: str) -> str:
        examples = "FEW SHOT BEHAVIOR EXAMPLES:\n" + EXAMPLES if chunk_index == 0 else "Use the previous pages as context."
    ...
    def _write_pages(self, pages: list[dict], page_map: dict[str, Path]) -> None:
        ...
        index_path.write_text("\n".join(f"[[{k}]] — {v}" for k, v in sorted(existing.items())), encoding="utf-8")


    def execute(self, pdfs) -> dict:
        if not pdfs:
            return {"status": "error", "message": "No PDFs provided"}

        results = []

        for pdf in pdfs:
            filename = getattr(pdf, "name", "source.pdf")
            text = self.extract_pdf_text(pdf.read_bytes())
            ...
            page_map = self._get_page_map()
            chunks = self.chunk_text(text)

            for i, chunk in enumerate(chunks):
                prompt = self._build_prompt(chunk, i, len(chunks), index, self._assign_pages(chunk, index), filename)
                response = "".join(self.stream_fn(prompt, system=SCHEMA if i == 0 else REUSE_SCHEMA))
